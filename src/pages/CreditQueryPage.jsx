...
export default function CreditQueryPage() {
  ...
  const getQueryStatusLabel = (status) => {
    const labels = {
      'pending': 'Pending',
      'sent': 'Sent to Bank',
    ...
    return labels[status] || status?.charAt(0).toUpperCase() + status?.slice(1) || 'Pending';
  };

  const handleShareWhatsApp = () => {
    if (creditQueries.length === 0) {
      setError('No credit queries to share');
      setTimeout(() => setError(''), 3000);
      return;
    }

    const name = selectedLead?.customerName || 'Lead';
    const loanType = (selectedLead?.loanType || '').replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase()) || 'Loan';
    const header = `📋 *Credit Queries - ${name} (${loanType})*\n\n`;

    // Group queries by bank
    const groupedByBank = {};
    creditQueries.forEach(q => {
      if (!groupedByBank[q.bank_name]) groupedByBank[q.bank_name] = [];
      groupedByBank[q.bank_name].push(q);
    });

    let body = '';
    Object.entries(groupedByBank).forEach(([bank, queries]) => {
      body += `*${bank}:*\n`;
      queries.forEach(q => {
        body += `• ${getQueryStatusLabel(q.status)} | ${(q.query_type || 'initial').charAt(0).toUpperCase() + (q.query_type || 'initial').slice(1)} | ${q.query_date ? new Date(q.query_date).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' }) : 'N/A'}\n`;
        if (q.remarks) body += `  Query: ${q.remarks}\n`;
        if (q.response_remarks) body += `  ✅ Response: ${q.response_remarks}\n`;
      });
      body += '\n';
    });

    const footer = `---\nTotal Queries: ${creditQueries.length}\nGenerated by InstaFin Portal`;
  ...
  const handleShareEmail = () => {
    ...
    const name = selectedLead?.customerName || 'Lead';
    const loanType = (selectedLead?.loanType || '').replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase()) || 'Loan';
    const subject = `Credit Queries - ${name} (${loanType})`;
    ...
    let body = `Credit Queries Report for ${name}\n`;
    body += `Loan Type: ${loanType}\n`;
    body += `Total Queries: ${creditQueries.length}\n`;
    body += `\n${'='.repeat(50)}\n\n`;
    ...
    Object.entries(groupedByBank).forEach(([bank, queries]) => {
      ...
      body += `${'-'.repeat(30)}\n`;
      queries.forEach((q, i) => {
        const statusLabel = getQueryStatusLabel(q.status);
        const typeLabel = (q.query_type || 'initial').charAt(0).toUpperCase() + (q.query_type || 'initial').slice(1);
        const dateStr = q.query_date
          ? new Date(q.query_date).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })
          : 'N/A';
        ...
        body += `  Date: ${dateStr}\n`;
        body += `  Query: ${q.remarks || 'N/A'}\n`;
        if (q.response_remarks) {
          body += `  Response: ${q.response_remarks}\n`;
          if (q.response_date) {
            body += `  Response Date: ${new Date(q.response_date).toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}\n`;
          }
        }
      });
      body += '\n';
    ...
    body += `${'='.repeat(50)}\n`;
    body += 'Generated by InstaFin Portal';
                ...
                {creditQueries.length > 0 && (
                  <>
                    <button
                      ...
                      className="px-3 py-2 bg-green-600 text-white rounded-xl font-semibold text-sm hover:bg-green-700 transition-all flex items-center gap-1.5"
                      title="Share queries via WhatsApp"
                    >
                    ...
                    <button
                      ...
                      className="px-3 py-2 bg-blue-600 text-white rounded-xl font-semibold text-sm hover:bg-blue-700 transition-all flex items-center gap-1.5"
                      title="Share queries via Email"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5">

                ...
                {!showBulkArea ? (
                  ...
                  <div className="border-2 border-dashed border-violet-300 rounded-2xl p-4 bg-violet-50/30">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <button onClick={handleBulkClear} className="text-xs text-gray-500 hover:text-red-600 font-medium">Cancel</button>
                    </div>
                    ...
                    <div className="space-y-3">
                      ...
                      {bulkFiles.length > 0 && !bulkPreview && (
                        <button onClick={handleSegregate} className="w-full py-2.5 bg-gradient-to-r from-violet-600 to-indigo-600 text-white rounded-xl font-semibold text-sm hover:from-violet-700 hover:to-indigo-700 transition-all shadow-sm flex items-center justify-center gap-2">
                              ...
                              <div className="space-y-1 max-h-32 overflow-y-auto">
                                {bulkPreview.matched.map((m, i) => (
                                  <div key={i} className="flex items-center gap-2 text-xs text-gray-700">
                                    ...
                                    <span className="text-green-700 bg-green-50 px-2 py-0.5 rounded-full flex-shrink-0">→ {m.documentName}</span>
                                  </div>
                                ))}
                              </div>
                              <button onClick={handleUploadAllMatched} disabled={bulkUploading}
                                className={`mt-2 w-full py-2 rounded-lg font-semibold text-sm transition-all flex items-center justify-center gap-2 ${bulkUploading ? 'bg-gray-300 text-gray-500 cursor-wait' : 'bg-green-600 text-white hover:bg-green-700'}`}>
                                {bulkUploading ? (
                                  <>
                          ...
                          {bulkPreview.unmatched.length > 0 && (
                            <div className="bg-white border border-amber-200 rounded-xl p-3">
                              <p className="text-xs font-bold text-amber-800 mb-2 flex items-center gap-1.5">
                                ...
                                {bulkPreview.unmatched.length} File(s) Unmatched
                              </p>
                              <p className="text-[10px] text-gray-500 mb-2 italic">Select a document type to assign manually.</p>
                                  ...
                                  <select
                                    onChange={(e) => {
                                      if (e.target.value) {
                                        ...
                                        if (selectedItem) handleManualAssign(u.file, selectedItem.id, selectedItem.name);
                                      }
                                    }}
                              ...
                              <button onClick={() => setBulkFiles([])} className="mt-1 text-xs text-amber-600 hover:text-amber-700 font-medium">Clear & try different files</button>
                            </div>
                          )}
                        </div>
                      )}

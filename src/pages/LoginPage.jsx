  ...
  return (
    <div className="landing jungle-theme">
      ...
      {/* =======================================================
          ...
          ======================================================= */}
      {showForgotPassword && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center z-50 p-4" onClick={() => setShowForgotPassword(false)}>
          <div
            className="bg-white rounded-3xl p-8 max-w-md w-full relative shadow-2xl animate-slide-up"
          ...
          >
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">Request a password reset</p>
              </div>
            </div>
            ...
            {!forgotSuccess ? (
              <>
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-4">
                  <p className="text-sm text-amber-800 font-medium">
                    Your request will be sent to the admin for approval. Once approved, the admin will set a new password for your account.
                  </p>
                </div>
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      ...
                      if (!res.ok) {
                        setForgotError(data.error || 'Failed to submit request');
                      } else {
                        setForgotSuccess(data.message);
                      }
                    } catch (err) {
                      setForgotError('Failed to submit request. Please try again.');
                    } finally {
                      setForgotLoading(false);
                    }
                  }}
                  disabled={forgotLoading}
                  className="w-full py-3 bg-gradient-to-r from-orange-500 to-orange-600 text-white rounded-xl font-bold hover:from-orange-600 hover:to-orange-700 disabled:opacity-50 transition-all shadow-md flex items-center justify-center gap-2"
                >
                  {forgotLoading ? (
                    <>
              ...
              <div className="text-center py-4">
                ...
                <p className="text-sm text-gray-600 leading-relaxed">
                  Forgot password request submitted successfully.<br />
                  <strong>Please contact the admin for new password.</strong>
                </p>
                <button
                  onClick={() => setShowForgotPassword(false)}
                  className="mt-6 px-6 py-2.5 bg-gray-100 rounded-xl font-semibold text-gray-700 hover:bg-gray-200 transition-colors"
                >
                  Close
                </button>
              </div>

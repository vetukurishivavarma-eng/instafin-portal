...
export default function CustomerLoginPage() {
  ...
  const handleAnalyzeSection = async (category) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Merge extracted details into the AI-verified profile
        try {
          const jsonMatch = data.summary.match(/```json([\s\S]*?)```/);
          if (jsonMatch && jsonMatch[1]) {
            const parsed = JSON.parse(jsonMatch[1].trim());
  ...
  // ===== Checklist Action Handlers (Download, Share) =====
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-5 sm:p-6">
            ...
            <div className="border border-gray-200 rounded-xl overflow-hidden mt-4">
              ...
              <div className="p-4">
                ...
                {checklistStatuses['other_docs'] && checklistStatuses['other_docs'].length > 0 && (
                  <div className="mt-3">
                    <p className="text-xs font-semibold text-gray-600 mb-2">Uploaded Other Documents:</p>
                    <div className="space-y-2">
                      {checklistStatuses['other_docs'].map((file) => (
                        <div key={file.id} className="flex items-center gap-2 bg-gray-50 rounded-lg px-3 py-2 border border-gray-100">
                          ...
                          <div className="flex items-center gap-1 flex-shrink-0">
                            <button
                              onClick={() => handleViewDocument(file.id, file.description || 'Other Document')}
                              className="text-xs text-blue-700 font-semibold bg-blue-100 px-2 py-1 rounded-lg hover:bg-blue-200"
                            >
                              View
                            </button>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              >
                <>
                  ...
                  Send Pending via WhatsApp ({pendingCount})
                </>
              </button>
            </div>
          </div>

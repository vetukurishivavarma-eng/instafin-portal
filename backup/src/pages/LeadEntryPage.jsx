...
export default function LeadEntryPage() {
  ...
  const handleSaveLead = async () => {
    if (!formData.customerName || !formData.mobile || !formData.loanType) {
      setError('Please fill in customer name, mobile number, and loan type');
      return;
    }
      ...
      {activeTab === 'new' && (
        <div className="bg-white rounded-3xl shadow-xl p-8 border">
          <div className="grid md:grid-cols-2 gap-6 mb-6">
            <input type="text" placeholder="Customer Name" className="border rounded-2xl px-4 py-3" value={formData.customerName} onChange={(e) => setFormData(p => ({...p, customerName: e.target.value}))} />
            <input type="tel" placeholder="Mobile Number" className="border rounded-2xl px-4 py-3" value={formData.mobile} onChange={(e) => setFormData(p => ({...p, mobile: e.target.value}))} />
          </div>

          <div className="grid md:grid-cols-2 gap-6 mb-6">
            <select className="border rounded-2xl px-4 py-3" value={formData.loanType} onChange={(e) => setFormData(p => ({...p, loanType: e.target.value}))}>
            ...
            <input type="text" placeholder="Expected Loan Amount" className="border rounded-2xl px-4 py-3" value={formData.expectedAmount} onChange={(e) => setFormData(p => ({...p, expectedAmount: e.target.value}))} />
          </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-3xl p-6 max-w-md w-full mx-4">
            ...
            <div className="flex gap-4 mt-6">
              <button onClick={() => setSelectedLead(null)} className="flex-1 bg-gray-200 text-gray-700 px-6 py-3 rounded-2xl font-semibold">
                Cancel
              </button>

/**
 * Mechanical JS port of src/data/checklists.ts (frontend), regenerated by a
 * small script rather than hand-copied to avoid transcription drift — see
 * the git history of this file for the exact command. Kept in sync with the
 * frontend file the same way documentCatalog.js is kept in sync with
 * src/utils/bulkDocMatcher.ts: there is no automated build step wiring these
 * together, re-run the conversion after editing src/data/checklists.ts.
 *
 * Why a copy instead of importing the .ts file directly: Node's type-
 * stripping (stable on the Node 24 this backend deploys with) still requires
 * explicit ".ts" extensions in every import specifier it resolves, and
 * checklists.ts imports './types' without one (valid under a bundler, not
 * under Node's ESM resolver) — confirmed failing directly. Rather than
 * editing frontend source to satisfy a backend runtime constraint, or adding
 * an experimental Node flag this backend's local dev tooling (Node 22)
 * doesn't need, a plain data copy was the smaller, more robust footprint.
 *
